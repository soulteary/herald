...
// It is kept for callers that cannot return an error; initialization failures
// panic instead of leaving a partially initialized service running.
...
func NewRouterWithClientAndHandlersE(redisClient *redis.Client, log *logger.Logger) (*RouterWithHandlers, error) {
	...
	// Initialize handlers. A configured provider that cannot be constructed
	// must abort startup rather than silently disabling that channel.
