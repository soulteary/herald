...
func TimestampWithinDrift(timestamp string, now time.Time, drift time.Duration) bool {
	...
	// seconds delta by time.Second. The old multiplication could overflow a
	// time.Duration and turn a far-future timestamp into a negative duration that
	// incorrectly passed the drift check.
