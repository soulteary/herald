...
3. **Stargate** ruft **Herald** `POST /v1/otp/challenges` auf (user_id, channel, destination, purpose). Herald erstellt eine Challenge in **Redis**, sendet den Code über **SMS**, **E-Mail** oder **herald-dingtalk** (Provider-Send).
...
| **Provider-Send** | Herald → Provider | `POST /v1/send` (HTTP) — Herald ruft SMS-/E-Mail-/DingTalk-Adapter auf; für DingTalk ruft Herald herald-dingtalk auf |
...
- **Herald ↔ Provider (z. B. herald-dingtalk)**: Optionaler API Key (`HERALD_DINGTALK_API_KEY`). DingTalk-Anmeldedaten existieren nur in herald-dingtalk; Herald speichert sie nie.
